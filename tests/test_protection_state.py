...
from bpfw.protection.state import AuthorityState, UnlockWindow, load_authority_state, save_authority_state


def test_protection_state_roundtrip(tmp_path: Path) -> None:
    state = AuthorityState(
        protection_enabled=True,
        os_lock_enabled=True,
        active_unlock_window=UnlockWindow(
            resource_id="project_blueprint",
            resource_path="bpfw/blueprint.yaml",
            scope="manual",
            operation="unlock",
    ...
    assert loaded.active_unlock_window.operation == "unlock"
