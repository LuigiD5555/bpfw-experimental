...
class _HierarchicalCodePreviewRenderer:
    ...
    def _find_target_node(self, module_ast: ast.Module) -> ast.AST | None:
        """Find the Python code node for the block being rendered."""
    ...
    def _direct_child_definitions(self, node: ast.AST) -> list[ast.AST]:
        """Get direct class/function children from one Python code node."""
    ...
    def _render_lines(
        ...
        """Show numbered source lines with folded children and call replacements."""
    ...
    def _display_purpose(self, child: ast.AST, symbol: str, kind: str) -> str:
        """Get display purpose for one child definition."""
    ...
    def _call_arguments(self, node: ast.Call) -> str:
        """Get real call-site arguments for a call expression."""
        arguments: list[str] = []
    ...
    def _source_segment(self, node: ast.AST) -> str:
        """Get source text for an Python code expression."""
...
def _has_body(node: ast.AST) -> bool:
    """Check whether an Python code node has a body list."""
    body = getattr(node, "body", None)
    return isinstance(body, list)


def _candidate_types_for_symbol_type(symbol_type: str) -> tuple[type[ast.AST], ...]:
    """Get Python code tree candidate types for a catalog symbol type."""
...
def _humanize_identifier(identifier: str) -> str:
    """Build a readable fallback from an identifier."""
